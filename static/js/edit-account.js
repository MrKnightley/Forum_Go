...
password1.onkeyup = checkPasswords;
...
function isValidUsername() {
    ...
    } else if (username.value.length != 0 && (username.value.length < 3 || username.value.length > 20)) {
...
function isLongEnough() {
    if (letterTyped.length != 0 && (letterTyped.length < 6 || letterTyped.length > 30)) {
        return false
    } else {
        return true
    }
